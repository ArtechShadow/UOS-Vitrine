...
Open at closeout: the Windows 11 / RTX 5090 acceptance run on real hardware remains the release gate.
The measured evidence so far is under budget on every path. The object pass's instrumented stage (identify)
...
stayed under 32\,GB reserved on Linux/Ada, Lane~A with roughly $3.5\times$ headroom; the Windows/5090
acceptance run is still required to confirm Blackwell fit.
...
key licence point: ReconViaGen v0.5 does not load the Facebook VGGT checkpoint --- it loads
...
is obtained. Lane~A (TRELLIS.2-4B, MIT) and everything else in the finishing path are clean. DINOv3 (\texttt{facebook/dinov3-vitl16-pretrain-lvd1689m}) conditions the TRELLIS.2 image encoding
in both lanes; its gated custom licence was accepted by the owner on 2026-08-28 and the checkpoint
downloaded at install. Every checkpoint used is recorded in lineage: repo, commit, hash and licence
class.
...
facebook/dinov3-vitl16 & gated custom & accepted 2026-08-28 \\
\bottomrule
\end{tabular}
\end{table}

Lane~B is productive here --- it produced single- and multi-view textured GLBs (\S\ref{sec:objgen}).
What is not yet reproduced is a clean-machine rebuild of ReconViaGen's pinned upstream environment
and a commercial-eligible checkpoint path; two blockers stand behind that. ReconViaGen pins conda py3.10 +
