...
fragmentary components. The fix is per-instance masks fed to carve separately, not per-label unions. The same fault has a scale consequence on nc04: the pass-2 radio subset spans roughly
$15.8\times3.1\times10.7$ units against a scene scale of 9.2, i.e. it swallows wall and floor, so the
carve centroid and extent are contaminated and object-to-scene placement stays approximate until the
carve is cleaned.
